...
// socket stuff
#define memset_addr 0x19d5c
#define close_addr 0x19ad4
#define open_addr 0x19198
#define mmap_addr 0x1981c

#define fileno_addr 0x199e4
#define ak_vi_set_fps_addr 0x706c8
...
struct video_input_frame {
	struct vi_frame_struct vi_frame[2]; // check dis
	void *mdinfo;
};
...
// make typedefs for all the functions we use
...
typedef int fileno_t(int* stream);
...
typedef void* mmap_t(void* addr, size_t length, int prot, int flags, int fd, size_t offset);
typedef void* ak_vi_set_fps_t(void* vi_handle, uint32_t fps);

// now for actual function definitions
...
void add_to_encode_list(struct thread_arg *thread_arg, struct frame_node *frame);
int frames_ctrl(int group_type, const int max_frame, unsigned long long ts);
...
int _start(void) {
    ...
    ak_vi_set_fps_t *ak_vi_set_fps = (ak_vi_set_fps_t*) ak_vi_set_fps_addr;
    ...
    printf(0x1, "Hello from dispatcher thread!\n");
    ...
    *arg_run_flag_addr = 0;
    ...
    printf(0x1, "Killing off pointless threads...\n");

    // kill off the sw_thread and qr_threads for some memory saving
    uint32_t* for_p2p_s_get_status = (uint32_t*) 0x522ab8;
    *for_p2p_s_get_status = 0;

    // kill off photosensitive thread as thats hogging cpu
    uint32_t* misc_ctrl_thread_run = (uint32_t*) 0x4b9b8c;
    *misc_ctrl_thread_run = 0;

    // kill off mt_ctrl_thread
    uint32_t* mt_ctrl_run_flag = (uint32_t*) 0x4b9584;
    *mt_ctrl_run_flag = 0;

    // lower the fps
    uint32_t* vi_handle_addr = (uint32_t*) 0x4b9a9c;
    ak_vi_set_fps(vi_handle_addr, 0xa);
    
    printf(0x1, "Reinitialising everything!\n");
...
void updateKeyStatus(char* key_status, uint32_t* current_key_timeout){
    ...
    *horizontal_position_addr = 180;
    ...
    // use is at index 5 of the buffer
    key_status[ANYKA_USE] = 0;

    // escape is at index 6 of the buffer
    key_status[ANYKA_ESCAPE] = 0;


    uint32_t *light_switch_addr = (uint32_t*) 0x4b9624;
    // printf(0x1, "Timeout is: %d\n", *current_key_timeout);
...
void* bootleg_yi_live_video_thread(yi_request_video* arg){
    ...
    fileno_t *fileno = (fileno_t*) fileno_addr;

    ak_thread_exit_t *ak_thread_exit = (ak_thread_exit_t*) ak_thread_exit_addr;

    printf(0x2, "Hello from bootleg_yi_live_video_thread thread!\n");
    ...
    *arg_run_flag_addr = 1;
    ...
    printf(0x2, "Setting run flag\n");
    uint32_t* ak_yi_av_run_flag_addr = (uint32_t*) 0x4b97a8;
    *ak_yi_av_run_flag_addr = 1;

    sleep(1);

    printf(0x2, "Current vi_run_flag: %d\n", *ak_yi_av_run_flag_addr);

    printf(0x2, "Entering loop\n");

	ak_venc_set_fps(arg->venc_handle, 0xa);

	uint32_t i_cnt = 0;

    int* file;
    int fd;
    char *key_status_shared;

    uint32_t current_key_timeout = 0;

    // initialise the key status mmap
    file = fopen(KEY_STATUS_FILENAME, "r+");
    if (file == NULL) {
        printf(0x2, "fopen error\n");
        ak_thread_exit();
    }
    ...
    fd = fileno(file);
    if (fd == -1) {
        printf(0x2, "fileno error\n");
        ak_thread_exit();
    }

    // Map the file into memory
    key_status_shared = mmap(NULL, KEY_STATUS_SIZE, 0x1 | 0x2, 0x1, fd, 0);
    if (key_status_shared == MAP_FAILED) {
        printf(0x2, "mmap error\n");
        ak_thread_exit();
    }

    while (arg->run_flag != 0){
        // update the key status
        updateKeyStatus(key_status_shared, &current_key_timeout);
...
static int start_service_work(void *vi)
{
    ...
    printf(0x1, "Starting up bootleg capture thread and normal encode thread...\n");
...
uint16_t htons(uint16_t hostshort) {
    ...
    if (check_endian.bytes[0] == 1 && check_endian.bytes[1] == 2) {
        // Host is big-endian, no need to swap bytes
        return hostshort;
    } else {
...
void *bootleg_capture_thread(void *arg){
    ...
    ak_thread_exit_t *ak_thread_exit = (ak_thread_exit_t*) ak_thread_exit_addr;
    ak_printf_t *printf = (ak_printf_t*) ak_print_addr;
    memset_t *memset = (memset_t*) memset_addr;
    ...
    int* file;
    int fd;
    char *yuv_buffer_shared;

    printf(0x3, "Hello from bootleg_capture_thread!\n");

    // initialise the shared yuv buffer
    file = fopen(YUV_BUFFER_FILENAME, "r+");
    if (file == NULL) {
        printf(0x3, "fopen error\n");
        ak_thread_exit();
    }
    ...
    while (thread_arg->cap_run && (video_ctrl.inited_enc_grp_num > 0)) {
        ...
        if (frame) {
            ...
            if (AK_SUCCESS == ret) {
                ...
                fast_memcpy(frame->vframe->vi_frame[1].data, mem_ptr, size);
